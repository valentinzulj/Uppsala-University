...
\noindent As can be seen, the moderator dampens the effect of \textit{negtone} on \textit{perform}. Actually, Table \ref{tab:mod} that, in the moderated model, the standalone effect of \textit{negtone} is far from being significant. The interaction term, however, does seem to have a significant relationship with the response variable. The point of this is that \textit{negtone} does not have a direct effect on \textit{perform}, but rather only effects the response through the way in which it -- \textit{negtone} -- interacts with \textit{negexp}.
...
\noindent where the upper first equation controls the mediator effect, and the second is just the ordinary linear model. I run the \texttt{mediate} function using linear models based on the equations, and get the results displayed in Table \ref{tab:med}. The table shows that \textit{dysfunc} affects the response i significantly in two ways, both directly and also through the effect is has on \textit{negtone}, which leads to an indirect effect on the response. 
...
In this task, I work with the \texttt{SEXDISC} data set. The data consits of four different variables, one of which contains missing values. In order to determine the extent -- and the type -- of the missingness, I make a series of plots that show how the \texttt{NA} terms are distributed across the variables. The illustrations of missingness is displayed in Figure \ref{fig:missing}.
...
\begin{tabular}{cccccc}
  ...
  \hline
  Intercept &  0.49 & 0.04 & 13.91 & 494.50 & 0.00 \\ 
  $X_{1}$   &  0.04 & 0.04 & 0.92 & 127.41 & 0.36  \\ 
  $X_{2}$   & -0.05 & 0.04 & -1.46 & 488.59 & 0.14  \\ 
  \hline
\end{tabular}
