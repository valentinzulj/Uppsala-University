...
bag <- function(B, train, test){
  ...
  for(i in 1:B){
    ...
    mod_sel <- step(mod, direction = "both", trace = FALSE)             # Model selection
...
f_bag <- function(B, train, test){
  ...
  dep <- "SalePrice"                        # Dependent variable
...
f_bag(1000, train, test)

#### Stacking ####
n <- nrow(train)
hat <- matrix(NA, nrow = n, ncol = 4)
colnames(hat) <-  c("Linear", "LASSO",
             "Ridge", "Step")
...
predictions <- cbind(intercept, test_lm, test_lass, test_rid, test_step)
