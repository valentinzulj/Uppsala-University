...
\noindent where $m = 1465$ and $\lambda = 0.05$.

Now, in order to save tim not having to run the simulations throught \texttt{knitr}, we compute the intervals in our ordinary \texttt{R} session, save the intervals in a \texttt{.csv}, and load it into this environment to make the graph. We do this as follows:
