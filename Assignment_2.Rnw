...
Bagging is a method for bootstrap aggregation which is based on fitting specially selected models to different bootstrap samples in order to predict the same response variable a large number of times. Doing so allows us to use the mean of these predictions as a final estimate of the response. This section will aim to write a function that performs bagging, and apply it to the data on house prices.
...
\noindent The computational burden of \texttt{bag} is rather heavy, and evaluating it using any meaningful value of \texttt{B} would be incredibly time consuming. Because of this, we turn to parallel computing in order to save time in running the function. To do this, we use the following code, rendering an RMSE value of 35 682:
...
print(models_rmse[1, 5] <- parallel)
...
Feature bagging is rather similar to the 'ordinary' bagging process discussed above. However, in feature bagging, we draw a random sample from the list of regressors and use them as covariates when estimating our model. Assuming we have a covariate matrix of dimensions $n \times p$, $p$ will be the number of covariates -- excluding the intercept. In feature bagging, it is common practice to draw a sample of $\sqrt{p}$ covariates for each bootstrap sample used.
...
b <- matrix(reg_y$coefficients, ncol = 1)   # Saving coefficients
@
...
\noindent Finally, we compute the root mean square error:
...
The final section of this report will give a comparison of how well the different models manage to predict the house prices given in our data set. The results will be presented in a figure, showing confidence intervals for the RMSE of each model. The intervals are given by

\begin{align*}
\left[ \sqrt{\frac{m}{\chi^{2}_{1-\alpha/2,m}}} RMSE, \; \sqrt{\frac{m}{\chi^{2}_{1-\alpha/2,m}}} RMSE \right],
\end{align*}

\noindent where $m = 1465$ and $\alpha = 0.05$.

Now, in order to save time not having to run the simulations through \texttt{knitr}, we compute the intervals in our ordinary \texttt{R} session, save the intervals in a \texttt{.csv} file, and load it into this environment to make the graph. We do this as follows:
...
rmse_final[, 1] <- c("Linear all", "Linear step", "Ridge", "LASSO", 
                     "Bagging", "Feature bagging", "Stacking")       # Output data frame

ch1 <- qchisq(0.975, df=1465)                                        # Chi-squared values
ch2 <- qchisq(0.025, df=1465)
for(i in 1:nrow(rmse_final)){
  rmse_final[i, 2] <- (sqrt(1465/ch1))*models_rmse[i]                # Lower limit
  rmse_final[i, 3] <- models_rmse[i]                                 # Point estimator
  rmse_final[i, 4] <- (sqrt(1465/ch2))*models_rmse[i]}               # Upper limit

write.csv(rmse_final, "rmse_final.csv")                              # Exporting .csv
@

\noindent With the \texttt{rmse\_final.csv} placed in our working directory, we can load it:

<<>>=
rmse_final <- read.csv("rmse_final.csv")
@

\newpage

\noindent And use it to make the plot mentioned above:

<<rmse, echo = FALSE, fig.height = 4, fig.cap = "Comparing RMSE for each model", fig.pos = "h!">>=
...
ggplot(rmse_final, aes(x = RMSE, y = names)) +
  geom_point(aes(size = 3, color = factor(X))) +
  geom_errorbarh(aes(xmax = upper, xmin = lower, color = factor(X)),
                 height = 0.4) +
  theme_classic() + 
  ...
  labs(y = NULL,
       x = expression(sqrt(MSE))) +
  guides(size = FALSE,
         colour = guide_legend(reverse=T)) +
  theme(panel.grid.major.x = element_line(color = "gray90"))
@

\noindent In Figure \ref{fig:rmse}, the point estimates and confidence intervals of the RMSE for each model is presented. We can now see that the the Ridge- and Stacking models are better than the Feature bagging model.
