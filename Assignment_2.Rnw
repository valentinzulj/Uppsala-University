...
\noindent \texttt{bag} is rather slow, and evaluating it using any meaningful value of \texttt{B} would be incredibly inefficient. Because of this, we turn to parallel computing in order to save time in running the function. To do this, we use the following code:
