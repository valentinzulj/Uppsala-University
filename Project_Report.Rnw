...
<<importance, echo = FALSE, fig.cap = "Plot showing the mean decrease in Gini index generated by each feature. Elevation and soil type look to be by far the most important", fig.height = 5>>=
imp %>%
  arrange(var) %>%
...
\noindent Furthermore, we conducted a minor simulation study in order to determine what effect tree depth has on the performance of a random forest model
