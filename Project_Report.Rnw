...
Finally, we intend to grow a classification tree to compare its performance to that of the random forest model. In order to grow the simple decision trees, we use a package called \texttt{rpart} \cite{R-rpart}. Random forests models will be described in Section \ref{sec:forest}. 
...
\noindent Giving us the baseline category logit model \cite{glm}. The parameters of the multinomial logit model are estimated is the maximum likelihood method, but the derivation of the estimators is left out of this report. However, we use the \textttt{multinom} function in the \texttt{nnet} package to implement it \cite{R-nnet}.
