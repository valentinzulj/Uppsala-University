...
In this task we were given three data files. We were asked to join the three data files, clean them and then do some analysis of the data. We begin by reading the data files:
...
colnames(county_facts)[1] <- "fips" # Assigning matching column manes
county_facts$fips <- as.character(county_facts$fips) # Need for same class
results$fips <- as.character(results$fips)           # to join
...
Since we were interested in the results of the Republican party, we remove the rows which have missing values regarding the Republican results. In doing so, we end up with 3112 observations.

<<>>=
sammanslaget <- na.omit(sammanslaget, cols = "per_gop_2016") 
@

We also create a new variable, gop\_win, which takes value 1 if the Republicans got more votes than the Democrats
...
the election result for the republican party in 2016 and 2012. We can see that the republican party did a better election in 2016 since there are more counties with higher results.
...
\newpage
Figure \ref{fig:boxplot1} shows two boxplots, one for the level of education in counties won by the democrats and one for the level of education in counties won by the republicans. We can see that the level of education tends to be higher in counties won by the democrats.
...
Figure \ref{fig:densityplot} shows the distribution of the share of elderly people in counties won by each party. We can see that the GOP generally won in counties where elderly people had a bigger share of the total population, perhaps indicating to Trump that his make America great again slogan worked well among those who are old enough to remember the "good old days".
...
The regression line in Figure \ref{fig:coolscatter} shows a weak, negative relationship between the level of high school graduation and support for the Republican party. Moreover, it seems that counties that have a low rate of high school graduation also tend to have a larger number of people in each household. This graph concludes the exploratory data analysis, and in the next couple of sections we will produce conditional and unconditional summaries of the data.
...
This section will provide three summaries where the data has not been restricted by any conditions, meaning that there is no grouping or filtering whatsoever.

We begin with a table showing a summary of per capita money income over the past 12 months. All values are given in the 2013 value of the US Dollar, and the sample was taken during the period stretching from 2009 to 2013. We cab see that the gap between the maximum and minimu income is strikingly big -- almost \$54,000 -- which might indeed be the reason why Trump seemed to do well among poorer voters.
...
Table \ref{tab:murder} shows a summary of the variable that counts the arrests and offenses related to murders. It shows that the median number of arrests or offences is 0, meaning that in at least half of the counties, no such event had occured. Perhaps, this could serve as an indicator that Trump tended to overdo his rhetoric regarding the crime rates, and the safety of most Americans.
\newpage
...
In Figure \ref{fig:wisp} we have plotted the mean number of firms against the share of women in the population, grouped by state. This means that every point in the scatterplot represents an individual state, as is shown by the labels attached to each point.
\newpage
