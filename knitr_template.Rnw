...
<<include=FALSE>>=
tab_sum_1 <- summary(sammanslaget$INC910213)
tab_sum_2 <- summary(sammanslaget$MURDER)
tab_sum_3 <- summary(sammanslaget$PST045214)
@

Table ??? shows a summary of the variable per capita money income in past 12 months (2013 dollars), 2009-2013. 

Table ??? shows a summary of the variable counts of arrests and offenses for the crime murder. We can see that most of the counties were free from murders.

Table ??? shows a summary of the variable population, 2014 estimate. We can see that more than half of the counties has a population of less than 26000. However, the mean is driven up by a few very big counties.

<<results = "asis", echo = FALSE>>=
xtable(tab_sum_1, caption = "My regression model")
@
